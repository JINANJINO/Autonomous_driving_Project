...
class Car:
    ...
    def __init__(self):
        ...
        self.INPUT_s_NOISE = np.deg2rad(5.0)
    ...
    def update_collision(self, env):
        is_collision, o = self.check_collision_vehicle(env)
        if is_collision:
            ...
            if x_size > y_size:
                if self.current_y > o[1]:
                    self.current_x = self.current_x
                    self.current_y = self.current_y + 3
                else:
                    self.current_x = self.current_x
                    self.current_y = self.current_y - 3
            else:
                if self.current_x > o[0]:
    ...
    ##############################################################
    def update_GPS(self):
        GPS_x = self.current_x + self.GPS_NOISE * np.random.randn(1)[0]*3
        GPS_y = self.current_y + self.GPS_NOISE * np.random.randn(1)[0]*3
        GPS_yaw = self.current_yaw + self.GPS_NOISE * np.random.randn(1)[0]*3

        GPS_x2 = self.current_x + self.GPS_NOISE * np.random.randn(1)[0]
        GPS_y2 = self.current_y + self.GPS_NOISE * np.random.randn(1)[0]
        GPS_yaw2 = self.current_yaw + self.GPS_NOISE * np.random.randn(1)[0]

        self.gps1 = [GPS_x, GPS_y, GPS_yaw]
        self.gps2 = [GPS_x2, GPS_y2, GPS_yaw2]
