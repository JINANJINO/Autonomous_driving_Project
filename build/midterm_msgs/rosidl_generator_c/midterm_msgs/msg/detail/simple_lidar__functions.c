  ...
  midterm_msgs__msg__SimpleLidar__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    midterm_msgs__msg__SimpleLidar * data =
      (midterm_msgs__msg__SimpleLidar *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!midterm_msgs__msg__SimpleLidar__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          midterm_msgs__msg__SimpleLidar__fini(&output->data[i]);
        }
        return false;
      }
    }
